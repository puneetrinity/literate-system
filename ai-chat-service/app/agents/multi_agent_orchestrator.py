...
class ResearchAgent(BaseAgent):
    def __init__(self, model_manager: ModelManager = None, cache_manager: CacheManager = None, search_graph=None):
    ...
    async def execute(self, task: AgentTask, state: GraphState) -> NodeResult:
        ...
        try:
            ...
            enable_web_search = task.input_data.get("enable_web_search", True)
            
            # Perform web search if enabled and appropriate for task type
            web_search_data = {"results": [], "search_performed": False}
            settings = get_settings()
            if (enable_web_search and 
                settings.enable_web_search_in_research and 
                task.task_type in ["fact_gathering", "trend_analysis", "information_gathering"]):
                logger.info(f"Performing web search for research query: {research_query}")
                web_search_data = await self._perform_web_search(research_query, task.task_type)
            # Build research prompt based on task type, including web search data
            if task.task_type == "literature_review":
                prompt = self._build_literature_review_prompt(
                    research_query, focus_areas, web_search_data
                )
            elif task.task_type == "fact_gathering":
                prompt = self._build_fact_gathering_prompt(research_query, web_search_data)
            elif task.task_type == "trend_analysis":
                prompt = self._build_trend_analysis_prompt(research_query, web_search_data)
            else:
                prompt = self._build_general_research_prompt(research_query, depth, web_search_data)
            ...
            if result.success:
                ...
                research_data = self._process_research_findings(
                    result.text, research_query, web_search_data
                )
                logger.debug(
                ...
                # Build methodology description including web search
                methodology = f"{depth} research using {model_name}"
                if web_search_data.get("search_performed"):
                    methodology += f" with web search ({len(web_search_data.get('results', []))} sources)"
                ...
                final_response = NodeResult(
                    ...
                    data={
                        ...
                        "sources_used": [
                            ...
                            for r in web_search_data.get("results", [])
                        ],
                        "citations": research_data.get("citations", []),
                        "sources_metadata": research_data.get("sources_metadata", {}),
                        "properly_cited": len(research_data.get("citations", []))
                    },
                    confidence=0.8,
    ...
    def _build_general_research_prompt(self, query: str, depth: str, web_search_data: dict = None) -> str:
        depth_instructions = {
        ...
        if web_search_data and web_search_data.get("search_performed") and web_search_data.get("results"):
            base_prompt += "RECENT WEB SEARCH RESULTS:\n"
            for i, result in enumerate(web_search_data["results"][:3], 1):
                base_prompt += f"{i}. {result['title']}\n"
                base_prompt += f"   URL: {result['url']}\n"
                base_prompt += f"   Summary: {result['snippet']}\n\n"
            base_prompt += "Use these recent findings to enhance your research.\n\n"
        
        base_prompt += (
            f"Please structure your research as follows:\n"
            f"1. Executive Summary\n2. Key Findings (numbered list)\n"
            f"3. Supporting Evidence\n4. Implications/Conclusions\n"
            f"5. Confidence Level (High/Medium/Low)\n\n"
            f"Focus on accuracy, credibility, and actionable insights. "
            f"Cite reasoning for your conclusions."
        )
        
        return base_prompt

    def _build_literature_review_prompt(self, query: str, focus_areas: list, web_search_data: dict = None) -> str:
        focus_text = ", ".join(focus_areas) if focus_areas else "general aspects"
        
        base_prompt = f"Conduct a literature review on: {query}\n\nFocus areas: {focus_text}\n\n"
        
        # Add web search data if available
        if web_search_data and web_search_data.get("search_performed") and web_search_data.get("results"):
            base_prompt += "RECENT LITERATURE AND SOURCES:\n"
            for i, result in enumerate(web_search_data["results"][:3], 1):
                base_prompt += f"{i}. {result['title']}\n"
                base_prompt += f"   Source: {result['url']}\n"
                base_prompt += f"   Abstract: {result['snippet']}\n\n"
            base_prompt += "Incorporate these recent sources into your literature review.\n\n"
        ...
        base_prompt += (
            ...
            f"Structure your review academically with clear evidence-based conclusions."
        )
        
        return base_prompt

    def _build_fact_gathering_prompt(self, query: str, web_search_data: dict = None) -> str:
        base_prompt = f"Gather and verify key facts about: {query}\n\n"
        
        # Add web search data if available
        if web_search_data and web_search_data.get("search_performed") and web_search_data.get("results"):
            base_prompt += "CURRENT FACTUAL SOURCES:\n"
            for i, result in enumerate(web_search_data["results"][:3], 1):
                base_prompt += f"{i}. {result['title']}\n"
                base_prompt += f"   Source: {result['url']}\n"
                base_prompt += f"   Facts: {result['snippet']}\n\n"
            base_prompt += "Use these current sources to gather and verify facts.\n\n"
        
        base_prompt += (
            f"Provide:\n1. Verified factual statements (numbered)\n"
            f"2. Statistical data (if available)\n3. Timeline of key events (if applicable)\n"
            f"4. Source reliability assessment\n5. Fact confidence ratings\n\n"
            f"Focus on accuracy and verifiability. Distinguish between facts, claims, and opinions."
        )
        
        return base_prompt

    def _build_trend_analysis_prompt(self, query: str, web_search_data: dict = None) -> str:
        base_prompt = f"Analyze trends related to: {query}\n\n"
        
        # Add web search data if available
        if web_search_data and web_search_data.get("search_performed") and web_search_data.get("results"):
            base_prompt += "CURRENT TREND DATA:\n"
            for i, result in enumerate(web_search_data["results"][:3], 1):
                base_prompt += f"{i}. {result['title']}\n"
                base_prompt += f"   Source: {result['url']}\n"
                base_prompt += f"   Trend Info: {result['snippet']}\n\n"
            base_prompt += "Use this current data to analyze trends.\n\n"
        
        base_prompt += (
            f"Provide:\n1. Current trend identification\n"
            f"2. Historical pattern analysis\n3. Driving factors behind trends\n"
            f"4. Future trend predictions\n5. Potential disruptions or changes\n\n"
            f"Support analysis with reasoning and evidence. Indicate confidence levels for predictions."
        )
        
        return base_prompt

    async def _perform_web_search(self, query: str, search_type: str = "general") -> dict:
        """Perform comprehensive web search using SearchGraph with enhanced processing"""
        if not self.search_graph:
            logger.info("SearchGraph not available, skipping web search")
            return {"results": [], "search_performed": False}
            
        try:
            # Import GraphState for search execution
            from app.graphs.base import GraphState
            
            # Enhance query based on search type
            enhanced_query = self._enhance_search_query(query, search_type)
            
            # Create a search state with appropriate settings for research
            search_state = GraphState(
                original_query=enhanced_query,
                query_id=f"research_search_{int(time.time())}",
                quality_requirement="high",  # Use high quality for research
                cost_budget_remaining=1.0,  # Allow higher cost for research
                max_execution_time=45.0     # Allow more time for comprehensive search
            )
            
            # Execute web search
            search_result = await self.search_graph.execute(search_state)
            ...
            if search_result.success and search_result.data:
                ...
                if "results" in search_data:
                    for result in search_data["results"][:8]:  # Get more results for research
                        processed_result = self._process_search_result(result, search_type)
                        if processed_result:
                            web_results.append(processed_result)
                
                # Analyze and rank results by relevance
                ranked_results = self._rank_search_results(web_results, query, search_type)
                ...
                return {"results": [], "search_performed": False, "error": search_result.error}
                
        except Exception as e:
            logger.error(f"Web search error: {str(e)}")
            return {"results": [], "search_performed": False, "error": str(e)}

    def _enhance_search_query(self, query: str, search_type: str) -> str:
        """Enhance search query based on research type"""
        enhancements = {
            "fact_gathering": f"{query} facts statistics data evidence",
            "trend_analysis": f"{query} trends analysis patterns recent developments",
            "literature_review": f"{query} research papers studies academic articles",
            "information_gathering": f"{query} comprehensive overview latest information"
        }
        
        return enhancements.get(search_type, query)

    def _process_search_result(self, result: dict, search_type: str) -> dict:
        """Process and enrich individual search result"""
        try:
            processed = {
                ...
                "relevance_indicators": self._extract_relevance_indicators(result, search_type),
                "timestamp_extracted": time.time()
            }
            
            # Only return if we have meaningful content
            if processed["title"] and (processed["snippet"] or processed["content"]):
                return processed
            return None
    ...
    def _classify_content_type(self, result: dict) -> str:
        """Classify the type of content for better handling"""
        title = result.get("title", "").lower()
        url = result.get("url", "").lower()
        snippet = result.get("snippet", "").lower()
        
        if any(indicator in title + url + snippet for indicator in ["research", "study", "paper", "journal"]):
            return "academic"
        elif any(indicator in title + url + snippet for indicator in ["news", "article", "report"]):
            return "news"
        elif any(indicator in title + url + snippet for indicator in ["data", "statistics", "chart", "graph"]):
            return "data"
        elif any(indicator in url for indicator in ["gov", "edu", "org"]):
            return "authoritative"
        else:
            return "general"

    def _extract_relevance_indicators(self, result: dict, search_type: str) -> list:
        """Extract indicators of content relevance for the search type"""
        indicators = []
        content = (result.get("title", "") + " " + result.get("snippet", "")).lower()
        
        # Type-specific relevance indicators
        type_keywords = {
            "fact_gathering": ["fact", "data", "statistic", "evidence", "proof", "confirmed"],
            "trend_analysis": ["trend", "increase", "decrease", "pattern", "growth", "decline", "forecast"],
            "literature_review": ["research", "study", "analysis", "findings", "conclusion", "methodology"],
            "information_gathering": ["overview", "comprehensive", "detailed", "complete", "thorough"]
        }
        
        keywords = type_keywords.get(search_type, [])
        for keyword in keywords:
            if keyword in content:
                indicators.append(keyword)
        
        # Temporal indicators for recent content
        temporal_keywords = ["2024", "2025", "recent", "latest", "current", "new", "updated"]
        for keyword in temporal_keywords:
            if keyword in content:
                indicators.append(f"temporal:{keyword}")
        
        return indicators

    def _rank_search_results(self, results: list, original_query: str, search_type: str) -> list:
        ...
        for result in results:
            ...
            relevance_count = len(result.get("relevance_indicators", []))
            score += relevance_count * 10
            ...
            if any(trusted in domain for trusted in ["edu", "gov", "org"]):
                score += 15
            elif any(news in domain for news in ["reuters", "bbc", "ap", "bloomberg"]):
                score += 10
            
            # Content length bonus (more comprehensive content)
    ...
    def _generate_citations(self, web_search_data: dict, research_text: str) -> tuple:
        """Generate proper citations from web search results"""
        citations = []
        sources_metadata = {}
        
        if not web_search_data.get("results"):
            return citations, sources_metadata
        
        # Process each web search result for citation
        for i, result in enumerate(web_search_data["results"][:5], 1):
            ...
            content_type = result.get("content_type", "general")
            timestamp = web_search_data.get("search_timestamp", time.time())
            
            # Generate citation in academic format
            citation_text = self._format_citation(title, url, domain, content_type, timestamp)
            ...
            citations.append({
                ...
                "used_in_research": self._check_source_usage(result, research_text)
            })
            
            # Store detailed metadata
            sources_metadata[citation_id] = {
                "title": title,
                "url": url,
                "domain": domain,
                "content_type": content_type,
    ...
    def _format_citation(self, title: str, url: str, domain: str, content_type: str, timestamp: float) -> str:
        """Format citation in academic style"""
        try:
            from datetime import datetime
            access_date = datetime.fromtimestamp(timestamp).strftime("%Y-%m-%d")
            
            # Basic citation format: Title. Domain. Accessed: Date. URL
            citation = f'"{title}". {domain.title()}'
            
            # Add content type indicator
            if content_type != "general":
                citation += f" ({content_type.title()})"
    ...
    def _check_source_usage(self, result: dict, research_text: str) -> bool:
        """Check if the source was likely used in the research text"""
        # Simple heuristic: check if key terms from the source appear in research
        source_terms = set()
        ...
        common_words = {"the", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by", "is", "are", "was", "were", "be", "have", "has", "had", "do", "does", "did", "will", "would", "could", "should", "may", "might", "can", "a", "an"}
        
        for word in title_words + snippet_words:
            if len(word) > 3 and word not in common_words:
                source_terms.add(word)
        
        # Check if significant terms appear in research text
        research_lower = research_text.lower()
        matches = sum(1 for term in source_terms if term in research_lower)
        
        # Consider source "used" if at least 2 meaningful terms match
        return matches >= 2

    def _process_research_findings(self, research_text: str, query: str, web_search_data: dict = None) -> dict:
        findings = []
        confidence = 0.7
        ...
        # Process web search data for citations
        citations = []
        sources_metadata = {}
        if web_search_data and web_search_data.get("search_performed"):
            citations, sources_metadata = self._generate_citations(web_search_data, research_text)
        ...
        return {
            ...
            "web_search_performed": web_search_data.get("search_performed", False) if web_search_data else False
        }
...
class MultiAgentOrchestrator:
    ...
    def __init__(self, model_manager: ModelManager, cache_manager: CacheManager, search_graph=None):
        self.model_manager = model_manager
        self.cache_manager = cache_manager
        self.search_graph = search_graph
    ...
    def create_agent(self, agent_type: AgentType) -> BaseAgent:
        ...
        # Pass SearchGraph to ResearchAgent for web search capabilities
        if agent_type == AgentType.RESEARCH_AGENT:
            return agent_cls(self.model_manager, self.cache_manager, self.search_graph)
        else:
