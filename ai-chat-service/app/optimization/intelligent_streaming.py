...
class IntelligentResponseStreamer:
    ...
    def get_streaming_config(self, routing_confidence: float, arm_performance: float) -> dict:
        """Get streaming configuration based on routing confidence and arm performance"""
        # Base configuration
        config = {
        ...
        if routing_confidence > 0.8:  # High confidence = faster streaming
            config.update({
                "initial_chunk_size": 5,
                "regular_chunk_size": 12,
                "delay": 0.03
            })
        elif routing_confidence > 0.6:  # Medium confidence = balanced streaming
            config.update({
                "initial_chunk_size": 3,
                "regular_chunk_size": 8,
                "delay": 0.05
        ...
        if arm_performance > 0.8:  # High performing arm
            config["delay"] *= 0.8  # Faster streaming
        elif arm_performance < 0.5:  # Low performing arm
            config["delay"] *= 1.5  # Slower streaming
        
        return config
    
    def create_adaptive_chunks(self, response_text: str, streaming_config: dict) -> list:
        """Create adaptive chunks based on streaming configuration"""
        words = response_text.split()
        chunks = []
        
        initial_chunk_size = streaming_config.get("initial_chunk_size", 3)
        regular_chunk_size = streaming_config.get("regular_chunk_size", 8)
        max_chunks = streaming_config.get("max_chunks", 100)
        
        i = 0
        chunk_count = 0
        
        while i < len(words) and chunk_count < max_chunks:
            # Use smaller chunks initially for faster perceived response
            chunk_size = initial_chunk_size if chunk_count < 3 else regular_chunk_size
    ...
    def get_adaptive_delay(self, chunk_index: int, total_chunks: int, streaming_config: dict) -> float:
        """Get adaptive delay based on chunk position and configuration"""
        base_delay = streaming_config.get("delay", 0.05)
        adaptive_delay = streaming_config.get("adaptive_delay", True)
        
        if not adaptive_delay:
            return base_delay
        
        # Adaptive delay strategy
        if chunk_index < 3:  # First few chunks stream faster
            return base_delay * 0.5
        elif chunk_index / total_chunks > 0.8:  # Last 20% stream slower for better reading
