    ...
    try:
        ...
        if cache_manager_app and session_id:
            try:
                ...
                if hasattr(cache_manager_app, 'get_from_l1_or_l2'):
                    cached_history = await cache_manager_app.get_from_l1_or_l2(
                        f"conversation_history:{session_id}"
                    )
                else:
                    # Fallback to basic get method
                    cached_history = await cache_manager_app.get(
                        f"conversation_history:{session_id}"
                    )
                if cached_history:
                    conversation_history = (
        ...
        # --- Accumulate conversation history for multi-turn BEFORE creating chat_data ---
        ...
        chat_data = ChatData(
            ...
            conversation_history=accumulated_history  # Set accumulated history directly
        )
