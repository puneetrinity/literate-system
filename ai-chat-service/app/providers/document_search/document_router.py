...
class DocumentSearchRouter:
    ...
    def analyze_query(self, query: str) -> QueryAnalysis:
        ...
        if not self.settings.enable_web_search_in_documents:
            ...
            query_type = "document"
            confidence = max(doc_score, 0.5)
            is_document_query = True
            if web_score > doc_score:
                reasoning.append("Web search disabled, using document search only")
        else:
            # Original logic with adjusted thresholds
            if hybrid_score > 0.7:
                suggested_provider = "hybrid"
                query_type = "hybrid"
                confidence = hybrid_score
                is_document_query = True  # Will use both systems
            elif doc_score > 0.3:  # Lowered from 0.5
                suggested_provider = "ultra_fast_search"
                query_type = "document"
                ...
                # Default to document search instead of hybrid
                suggested_provider = "ultra_fast_search"
                query_type = "document"
                confidence = 0.5
                is_document_query = True
                reasoning.append("Defaulting to document search")
