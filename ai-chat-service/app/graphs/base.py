...
class GraphState:
    ...
    @staticmethod
    def _reduce_search_results(current: List[Dict[str, Any]], new: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Reducer for search_results to handle concurrent updates"""
        if not current:
            return new
        if not new:
            return current
        
        # Merge and deduplicate based on URL/ID
        combined = current.copy()
        existing_ids = {item.get('url', item.get('id', str(i))) for i, item in enumerate(current)}
        
        for item in new:
            item_id = item.get('url', item.get('id', str(len(combined))))
    ...
    @staticmethod
    def _reduce_intermediate_results(current: Dict[str, Any], new: Dict[str, Any]) -> Dict[str, Any]:
        """Reducer for intermediate_results to handle concurrent updates"""
        if not current:
            return new
        if not new:
            return current
    ...
    @staticmethod
    def _reduce_errors(current: List[str], new: List[str]) -> List[str]:
        """Reducer for errors to handle concurrent updates"""
        if not current:
            return new
        if not new:
            return current
    ...
    @staticmethod
    def _reduce_warnings(current: List[str], new: List[str]) -> List[str]:
        """Reducer for warnings to handle concurrent updates"""
        if not current:
            return new
        if not new:
            return current
        ...
        async with self._state_lock:
            for key, value in updates.items():
                if hasattr(self, key):
                    ...
                    # Use reducers for fields that have them
