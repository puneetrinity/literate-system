...
class ResponseGeneratorNode(BaseGraphNode):
    ...
    def _build_prompt(self, state: GraphState) -> str:
        ...
        Now includes RAG context when available.
        """
        query = state.processed_query or state.original_query
        ...
        rag_section = self._build_rag_context(state)
    ...
    def _build_conversation_context(self, state: GraphState) -> str:
        ...
        for entry in recent_history:
            ...
            elif 'user_message' in entry and 'assistant_response' in entry:
                # New format
                user_message = entry.get('user_message', '')
                assistant_response = entry.get('assistant_response', '')
                context_lines.append(f"User: {user_message}")
                context_lines.append(f"Assistant: {assistant_response}")
        
        result = "\n".join(context_lines)
        logger.debug(f"Built conversation context: {result[:200]}...")
        return result
    ...
    def _build_rag_context(self, state: GraphState) -> str:
        ...
        """
        rag_enhancement = state.intermediate_results.get("rag_enhancement", {})
        if not rag_enhancement or not rag_enhancement.get("rag_sources"):
            return ""
        
        sources = rag_enhancement["rag_sources"]
        confidence = rag_enhancement.get("rag_confidence", 0.0)
        search_type = rag_enhancement.get("search_type", "hybrid")
        
        context_lines = [
            f"Relevant information found (confidence: {confidence:.1f}, search: {search_type}):"
        ]
        
        for source in sources:
            context_lines.append(f"Source {source['index']}: {source['title']}")
            context_lines.append(f"Content: {source['content']}")
            context_lines.append(f"Relevance: {source['score']:.2f}")
            context_lines.append("")  # Empty line between sources
        
        context_lines.append("Please use this information to provide an accurate, well-sourced response. "
                           "Cite specific sources when relevant.")
        
        result = "\n".join(context_lines)
        logger.debug(f"Built RAG context with {len(sources)} sources")
        return result
...
class ChatGraph(BaseGraph):
    ...
    def define_nodes(self) -> Dict[str, BaseGraphNode]:
        ...
        from app.graphs.rag_orchestration_node import RAGOrchestrationNode
        ...
        return {
            ...
            "rag_orchestration": RAGOrchestrationNode(),  # RAG enhancement node
    ...
    def define_edges(self) -> List[tuple]:
        ...
        Now includes RAG orchestration in the flow.
        """
        return [
            ("start", "context_manager"),  # Entry edge for LangGraph
            ("context_manager", "intent_classifier"),
            ("intent_classifier", "rag_orchestration"),  # RAG enhancement after intent classification
            ("rag_orchestration", "response_generator"),  # RAG-enhanced response generation
...
async def create_chat_graph(
    ...
    **kwargs
    ...
    # ChatGraph doesn't accept router parameter, it's handled elsewhere
    return ChatGraph(
