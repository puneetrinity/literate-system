...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        # Memory Orchestrator (optional, depends on model_manager and cache_manager)
        async def init_memory_orchestrator():
            from app.memory.memory_init import create_memory_orchestrator
            
            memory_orchestrator = await create_memory_orchestrator(
                model_manager=app_state["model_manager"],
                cache_manager=app_state["cache_manager"],
        ...
        # Initialize memory orchestrator if enabled
        if os.getenv("ENABLE_MEMORY_SYSTEM", "false").lower() == "true":
            memory_orchestrator = await monitor.initialize_component(
                "memory_orchestrator", init_memory_orchestrator
            )
            app_state["memory_orchestrator"] = memory_orchestrator
        else:
            memory_orchestrator = None
            logger.info("Memory system disabled - skipping memory orchestrator initialization")

        # Router Migration Manager (depends on model_manager, cache_manager, and optionally memory_orchestrator)
        async def init_migration_manager():
            from app.routers.migration_manager import create_router_migration_manager
            
            migration_manager = await create_router_migration_manager(
                model_manager=app_state["model_manager"],
                cache_manager=app_state["cache_manager"],
        ...
        # Only initialize migration manager if router migration is enabled
        enable_migration = os.getenv("ENABLE_ROUTER_MIGRATION", "true").lower() == "true"
        if enable_migration:
            migration_manager = await monitor.initialize_component(
                "migration_manager", init_migration_manager
            )
            app_state["migration_manager"] = migration_manager
        else:
            logger.info("Router migration disabled - skipping migration manager initialization")
...
app.include_router(web_search_consent.router, prefix="/api/v1/web-search/consent", tags=["Web Search Consent"])
app.include_router(web_search_billing.router, prefix="/api/v1/web-search/billing", tags=["Web Search Billing"])
app.include_router(web_search_audit.router, prefix="/api/v1/web-search/audit", tags=["Web Search Audit"])
