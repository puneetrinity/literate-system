...
# Global RAG system manager instance
_rag_manager = None
...
class RAGSystemManager:
    ...
    async def initialize(self, embedding_dim: int = 384, use_gpu: bool = False):
        ...
        """
        try:
            from app.rag.models import DocumentProcessor, EnhancedDocumentChunker, DocumentStore
            from app.rag.enhanced_engine import RAGUltraFastEngine
            from app.logger import get_enhanced_logger
            
            logger = get_enhanced_logger(__name__)
            logger.info("Initializing RAG system with Enhanced Chunking...")
            
            # Initialize document processor
            self.document_processor = DocumentProcessor()
            
            # Initialize enhanced document chunker
            self.document_chunker = EnhancedDocumentChunker(
...
async def get_rag_manager() -> Optional[RAGSystemManager]:
